...
const Produto = () => {
  ...
  // variável que guarda todos os produtos
    const [messages, setProducts] = useState<Message[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const handleRemoveProduct = async (id: Number) => {
       ...
       await fetch(`http://localhost:3333/message/${id}`, {
        ...
        setProducts(messages.filter((message) => message.id !== id));
      };
      ...
      <table className="w-full border border-gray-300">
        ...
        <tbody>
          ...
          {messages.map((message) => (
            <tr key={message.id}>
              <td className="border-b border-gray-300 py-2 px-4">{message.name}</td>
              <td className="border-b border-gray-300 py-2 px-4">{message.description}</td>
              <td className="border-b border-gray-300 py-2 px-4">{message.quantity}</td>
              <td className="border-b border-gray-300 py-2 px-4">{message.price}</td>
              ...
              <td className="border-b border-gray-300 py-2 px-4">
                <button onClick={() => handleBuyProduct(message.id)} className="flex items-center justify-center p-2 text-green-500 hover:text-green-700">
